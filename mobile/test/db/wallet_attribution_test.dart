...
void main() {
  ...
  /// پیامکِ بی‌شماره خودکار ثبت نمی‌شود (قانون)؛ فقط با «ثبت کن»ِ دستی، که مستقیم
  /// saveParsed است — انتسابِ صاحب در آن مسیر هم باید درست باشد.
  Future<String?> saveOwner(String sender, String body) async {
    final outcome = await repo.saveParsed(const SmsParser().parse(sender: sender, body: body),
        sender: sender, receivedAt: DateTime.utc(2026, 9, 10));
    return (await repo.getById(outcome.id))!.ownerName;
  }
  ...
  test('پیامکِ بی‌شماره به کارتی می‌رسد که فقط با بانک ثبت شده', () async {
    ...
    expect(await saveOwner('Blu', bluSms), 'مامان');
  });

  test('تعیین صاحب بعد از رسیدن پیامک‌ها: تراکنش‌های قبلی هم به همان کارت می‌روند',
      () async {
    expect(await saveOwner('Blu', bluSms), isNull);
    await repo.addWallet(
        const Wallet(id: '', ownerName: 'مامان', label: 'حساب بلو', bankId: 'blu'));
